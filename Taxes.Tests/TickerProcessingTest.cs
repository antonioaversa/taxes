...
public class TickerProcessingTest
{
    ...
    public void ProcessTicker_BuyLimit_IncreasesTotalQuantityAndAmountAccordingly()
    {
        ...
        List<Event> e = [new(T0, BuyLimit, Ticker, 3, 100, 303, 3, EUR, 1, Broker)];
        ...
        e.Add(new(T0 + 1 * D, BuyLimit, Ticker, 2, 110, 222, 2, EUR, 1, Broker));
        Instance.ProcessTicker(Ticker, e).AssertDefaultExceptFor(
            totalQuantity: 5, totalAmountBase: 525, portfolioAcquisitionValueBase: 525);
        e.Add(new(T0 + 2 * D, BuyLimit, Ticker, 1, 90, 91, 1, EUR, 1, Broker));
        Instance.ProcessTicker(Ticker, e).AssertDefaultExceptFor(
            totalQuantity: 6, totalAmountBase: 616, portfolioAcquisitionValueBase: 616);
        e.Add(new(T0 + 2 * D, BuyLimit, Ticker, 1, 90, 92, 2, EUR, 1, Broker));
        Instance.ProcessTicker(Ticker, e).AssertDefaultExceptFor(
    ...
    {
        List<Event> e = [new(T0, BuyLimit, Ticker, 3, 100, 303, 3, USD, 1.2m, Broker)];
        Instance.ProcessTicker(Ticker, e).AssertDefaultExceptFor(
    ...
    {
        ThrowsAny<Exception>(() => Instance.ProcessTicker(Ticker, [new(T0, BuyLimit, Ticker, -3, 100, 303, 3, EUR, 1, Broker)]));
        ThrowsAny<Exception>(() => Instance.ProcessTicker(Ticker, [new(T0, BuyLimit, Ticker, 0, 100, 303, 3, EUR, 1, Broker)]));
    }
    ...
    public void ProcessTicker_BuyMarket_Fees()
    {
        ...
        List<Event> e = [new(T0, BuyMarket, Ticker, 5, 126.21m, 632.62m, 1.58m, EUR, 1, Broker)];
        // TODO: continue
    }
    ...
    public void ProcessTicker_SellWithoutBuying_RaisesException() => 
        Instance.ProcessTicker(Ticker, [new(T0, SellLimit, Ticker, 3, 100, 303, 3, EUR, 1, Broker)]);

    [TestMethod]
    [ExpectedException(typeof(Exception), AllowDerivedTypes = true)]
    public void ProcessTicker_SellMoreThanOwned_RaisesException() =>
         Instance.ProcessTicker(Ticker, [
             new(T0, BuyLimit, Ticker, 3, 100, 303, 3, EUR, 1, Broker),
             new(T0, SellLimit, Ticker, 4, 100, 404, 4, EUR, 1, Broker)]);
    ...
    public void ProcessSell_WhenCurrenciesAreEtherogenousNotInSellOrBuy_IsOk()
    {
        ...
        var cashTopUp1 = new Event(T0 + 1 * D, CashTopUp, Ticker, 0, 0, 100, 0, USD, 1, Broker);
        // Selling 1 share at 2.0 EUR, with fees of 0.2 EUR => Total Amount Local of 1.8 EUR
    ...
    {
        // While owning 3 shares for a total of 5.5 EUR
        var buy1 = new Event(T0 + 0 * D, BuyLimit, Ticker, 3, 0, 0, 0, EUR, 1, Broker);
    ...
    {
        // While owning 3 shares for a total of 5.5 EUR
        var buy1 = new Event(T0 + 0 * D, BuyLimit, Ticker, 3, 2, 5.5m, 0.5m, EUR, 1, Broker);
    ...
    {
        // While owning 3 shares for a total of 5.5 EUR
        var buy1 = new Event(T0 + 0 * D, BuyLimit, Ticker, null, 2, 5.5m, 0.5m, EUR, 1, Broker);
        // Selling 1 share at 2.0 EUR, with fees of 0.2 EUR => Total Amount Local of 1.8 EUR
        var sell1 = new Event(T0 + 0 * D, SellLimit, Ticker, 1, 2.0m, 1.8m, 0.2m, EUR, 1, Broker);
        var state = new TickerState(Ticker, Isin, TotalQuantity: 3, TotalAmountBase: 5.5m);
        ThrowsAny<Exception>(() => Instance.ProcessSell(sell1, [buy1, sell1], 1, state, NoOuts));
    ...
    public void ProcessStockSplit_WhenTickerEventAndIndexAreInconsistent_RaisesException()
    {
        ...
        var events = new List<Event>
        {
            new(T0, BuyLimit, Ticker, 10, 100m, 1020m, 20m, localCurrency, fxRate, Broker),
    ...
    // TODO: add test on multi-country dividends and interests
