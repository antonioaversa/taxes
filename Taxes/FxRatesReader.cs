...
public partial class FxRatesReader(Basics basics)
{
    ...
    /// </summary>
    public FxRates ParseSingleCurrencyFromFile(string currency, string path) =>
    ...
    /// 31/03/2024;-;-;-;-;-;-;;-;-;;-;-;;-;-;-;-;-;-;-;;;;-;-;-;-;-;-;-;;-;-;;;-;-;-;-
    /// </example>
    public FxRates ParseMultiCurrenciesFromFile(string path) =>
        ParseMultiCurrenciesFromContent(File.ReadAllLines(path));

    internal /* for testing */ FxRates ParseSingleCurrencyFromContent(string currency, string[] lines)
    {
        ...
        foreach (var line in lines)
        {
            ...
            if (!DateTime.TryParseExact(fields[0], "M/d/yyyy", basics.DefaultCulture, DateTimeStyles.AssumeLocal, out var date))
                throw new InvalidDataException($"Invalid line: '{line}'");
            ...
            if (!decimal.TryParse(fields[1], NumberStyles.Currency, basics.DefaultCulture, out var fxRate))
                throw new InvalidDataException($"Invalid line: '{line}'");
            if (fxRate <= 0)
                throw new InvalidDataException($"Invalid line: '{line}'");
    ...
    {
        if (lines.Count < basics.FxRatesHeaderLinesFirstWord.Length ||
            basics.FxRatesHeaderLinesFirstWord.Where((s, i) => !lines[i].Trim().StartsWith(s)).Any())
        ...
        for (var lineIndex = basics.FxRatesHeaderLinesFirstWord.Length; lineIndex < lines.Count; lineIndex++)
        {
            ...
            for (var fieldIndex = 1; fieldIndex < lineFields.Length; fieldIndex++)
            {
                ...
                var fxRate = decimal.Parse(lineFields[fieldIndex], basics.FxRatesMultiCurrenciesCulture);
